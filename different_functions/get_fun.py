...
def get_symptoms(i):
    ...
    symptoms = set(doc_description[i][random.randrange(0, len(doc_description[i]))] for _ in prange(count_of_symptoms))
    return symptoms
...
def get_analysis_and_bill(i):
    ...
    analyses = set(doc_analyses[i][random.randrange(0, len(doc_analyses[i]))] for _ in prange(count_of_analyses))
    if "нет" in analyses:
        price = int(doc_prices[i])
    else:
        price = sum(random.randint(250, 1000) for _ in prange(count_of_analyses)) + int(doc_prices[i])
    return analyses, price-price % 10
